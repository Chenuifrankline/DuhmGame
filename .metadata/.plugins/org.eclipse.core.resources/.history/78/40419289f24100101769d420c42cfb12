...
'''.toString.trim()
	 ...
	 */
	def String removeExtraUnderscore(String code) {
		return code// Entfernt führende Unterstriche bei Funktionsaufrufen wie _DuhmGame_run
		.replaceAll("\\b_([A-Za-z][A-Za-z0-9]*)", "$1")// Ersetzt CamelCase wie DuhmGame_run mit Duhm_Game_run
		.replaceAll("\\b( Duhm)([A-Z])", "$1_$2")// Fixes main function prototype
		.replaceAll("int main\\(\\)", "int main(void)")// Entfernt eventuell doppelte Unterstriche
		.replaceAll("__", "_")// Optional: normalize whitespace
		.replaceAll("[ \\t]+", " ").trim
	}
